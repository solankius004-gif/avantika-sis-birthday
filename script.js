// ====== BIRTHDAY DATE ======
const birthdayDate = new Date("September 10, 2026 00:00:00").getTime();
...
function updateCountdown() {
  ...
  if (diff <= 0) {
    ...
    $("birthdayToday")?.classList.add("show");
    return;
  }

  $("birthdayToday")?.classList.remove("show");
...
function particleBurst(count = 45) {
  const symbols = ["💜","💖","✨","🎉","🌸","💎"];
...
setInterval(() => particleBurst(2), 1200);

$("surpriseBtn").addEventListener("click", () => {
  document.querySelector("#surprise").scrollIntoView({behavior:"smooth"});
  setTimeout(() => {
    particleBurst(80);
...
$("letterBtn").addEventListener("click", () => {
  ...
  $("letterBtn").textContent = $("letterCard").classList.contains("show")
    ? "💜 CLOSE THE LITTLE LETTER"
    : "💌 OPEN YOUR LITTLE LETTER";
  particleBurst(25);
});

let blessingsShown = false;
$("blessingBtn").addEventListener("click", () => {
  const blessings = document.querySelectorAll(".blessing");

  if (!blessingsShown) {
    blessingsShown = true;
    $("blessingBtn").textContent = "💜 BLESSINGS FOR YOU";

    blessings.forEach((item, index) => {
      setTimeout(() => item.classList.add("show"), index * 450);
    });

    setTimeout(() => {
      $("memoryReveal").classList.add("show");
      $("memoryReveal").scrollIntoView({behavior:"smooth", block:"center"});
    }, blessings.length * 450 + 600);

    particleBurst(35);
  } else {
    blessings.forEach(item => item.classList.remove("show"));
    blessingsShown = false;
    $("blessingBtn").textContent = "🤲 TAP FOR A LITTLE BLESSING";
  }
});
...
musicBtn.addEventListener("click", () => {
  if (!playing) {
    music.play().then(() => {
      ...
      alert("Put your song file at music/birthday.mp3 first.");
    });
  } else {
    music.pause();
    playing = false;
